...
export default function HomeClient() {
  ...
  async function handleSourcesAndChat(question: string) {
    ...
    setTimeout(async () => {
      try {
        await fetch("/api/dashboard/conversations", {
          ...
          body: JSON.stringify({
            topic: question.substring(0, 100), // Use first 100 chars as topic
            messages: messages.slice(1).map(msg => ({ // Skip system message
              role: msg.role,
              content: msg.content,
    ...
    }, 2000); // Wait 2 seconds for messages to be fully loaded
  }
